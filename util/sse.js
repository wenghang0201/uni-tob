...
class SSEService {
  ...
  connect(endpoint = '/sse/connect', options = {}) {
    ...
    try {
      ...
      // 将token作为URL参数传递，因为EventSource不支持自定义headers
  ...
  setupEventListeners() {
    ...
    this.eventSource.onmessage = (event) => {
      try {
        ...
        if (data.type) {
          ...
          // 当type为2的时候播放音效（检查用户设置）
          if (data.type === 2) {
            console.log('收到订单通知，检查声音设置');
            const soundEnabled = uni.getStorageSync('orderSoundEnabled');
            if (soundEnabled !== false) { // 默认开启，除非明确关闭
              console.log('播放订单音效');
    ...
    this.eventSource.onerror = (event) => {
      console.error('SSE连接错误 - onerror事件触发');
      ...
      if (event.target && event.target.readyState === EventSource.CLOSED) {
        console.error('SSE连接被关闭，可能是MIME类型错误');
        // uni.showToast({
      ...
      // 确保handleConnectionError方法被调用
      if (typeof this.handleConnectionError === 'function') {
        this.handleConnectionError();
      } else {
        console.error('handleConnectionError方法不存在');
      }
    };
  }
    ...
    if (typeof document !== 'undefined') {
      document.addEventListener('visibilitychange', () => {
        if (document.hidden) {
          ...
          console.log('页面回到前台，SSE连接状态:', this.isConnected);
        }
      });
    }
  }
      ...
      if (typeof uni !== 'undefined' && uni.createInnerAudioContext) {
        ...
        this.audioContext.onError((res) => {
          ...
          if (res.errCode === 10001) {
            console.log('音频播放被浏览器限制，需要用户交互');
          }
        });
        
        this.audioContext.onEnded(() => {
          console.log('订单音效播放结束');
        });
  ...
  processPendingPlayQueue() {
    if (this.pendingPlayQueue.length > 0) {
      console.log(`处理待播放队列，共 ${this.pendingPlayQueue.length} 个待播放`);
  ...
  playOrderSound() {
    try {
		if (window.NativeBridge) 
		{
			//原生环境
			// 播放音乐
			window.NativeBridge.playMusic("https://a.kull.fun/audio/new_order.mp3");
		}else{
			// 如果音频未初始化，先初始化
			if (!this.audioContext && !this.audioInitialized) {
			  console.log('音频未初始化，添加到待播放队列');
